...
export function createTerminal({
  ...
  let timer,
    fadeTimer,
    remaining = timing.historyLifetime || 30000,
    deadline;
  const collapse = () => {
  ...
  const schedule = () => {
    ...
    if (!document.hidden) timer = setTimeout(fade, remaining);
  };
  const append = (line) => {
    ...
    if (typeof line === "string") row.textContent = line;
      ...
      row.textContent = `${line.name.padEnd(line.width || 0)} - ${line.text}`;
    history.append(row);
  };
  const runCommand = async (raw) => {
    ...
    for (const command of splitCommands(raw)) {
      ...
      try {
        ...
        const lines = result?.then ? await result : result;
        for (const line of lines || []) append(line);
      } catch {
        append("Unable to load the archive. Please try again.");
      }
    }
